...
#define dbgResult(code) \
    qDebug() << QString::number(code) << code << " msg: " << gs_pDetInstance->GetErrorInfo(code).c_str()
...
void SDKCallbackHandler(int nDetectorID, int nEventID, int nEventLevel, const char* pszMsg, int nParam1, int nParam2,
                        int nPtrParamLen, void* pParam)
{
    qDebug() << "nEventID: " << nEventID << " nEventLevel: " << nEventLevel << " pszMsg: " << pszMsg
             << " nParam1: " << nParam1 << " nParam2: " << nParam2 << " nPtrParamLen: " << nPtrParamLen
             << " pParam: " << pParam;
    ...
    switch (nEventID)
    {
        ...
        case Evt_Image:
        {
            ...
            qDebug() << "Attr_CurrentTransaction: " << currentTransaction << " pImg->nWidth: " << pImg->nWidth
                     << " pImg->nHeight: " << pImg->nHeight << " pImg->nBytesPerPixel: " << pImg->nBytesPerPixel
                     << " nImageSize: " << nImageSize << " nFrameNo: " << nFrameNo << " nImageID: " << nImageID
                     << " nAvgValue: " << nAvgValue << " nCenterValue: " << nCenterValue;
...
NDT1717MA::NDT1717MA(QObject* parent) : QObject(parent)
{
...
bool NDT1717MA::GetAttr(int nAttrID, int& nVal)
{
    ...
    qDebug() << "nAttrID: " << nAttrID << " result: " << result
             << " msg: " << gs_pDetInstance->GetErrorInfo(result).c_str() << " nVal: " << nVal;
...
bool NDT1717MA::GetAttr(int nAttrID, float& fVal)
{
    ...
    qDebug() << "nAttrID: " << nAttrID << " result: " << result
             << " msg: " << gs_pDetInstance->GetErrorInfo(result).c_str() << " fVal: " << fVal;
    return result == Err_OK;
}
...
bool NDT1717MA::GetAttr(int nAttrID, std::string& strVal)
{
    ...
    qDebug() << "nAttrID: " << nAttrID << " result: " << result
             << " msg: " << gs_pDetInstance->GetErrorInfo(result).c_str() << " strVal: " << strVal.c_str();
    return result == Err_OK;
}
...
bool NDT1717MA::SetAttr(int nAttrID, int nValue)
{
    ...
    qDebug() << "nAttrID: " << nAttrID << " result: " << result
             << " msg: " << gs_pDetInstance->GetErrorInfo(result).c_str() << " nValue: " << nValue;
    return result == Err_OK;
}
...
bool NDT1717MA::SetAttr(int nAttrID, float fValue)
{
    ...
    qDebug() << "nAttrID: " << nAttrID << " result: " << result
             << " msg: " << gs_pDetInstance->GetErrorInfo(result).c_str() << " fValue: " << fValue;
    return result == Err_OK;
}
...
bool NDT1717MA::SetAttr(int nAttrID, const char* strValue)
{
    ...
    qDebug() << "nAttrID: " << nAttrID << " result: " << result
             << " msg: " << gs_pDetInstance->GetErrorInfo(result).c_str() << " strValue: " << strValue;
    return result == Err_OK;
}
...
bool NDT1717MA::UpdateMode(std::string mode)
{
    ...
    qDebug() << " 当前工作模式：" << QString::fromStdString(mode) << " Attr_Width: " << m_status.Width
             << " Attr_Height: " << m_status.Height << " Attr_AcqParam_Binning_W: " << m_status.AcqParam_Binning_W
             << " Attr_AcqParam_Zoom_W: " << m_status.AcqParam_Zoom_W << " SW_PreOffset: " << m_status.SW_PreOffset
             << " SW_Gain: " << m_status.SW_Gain << " SW_Defect: " << m_status.SW_Defect;
...
{
    qDebug() << "nCmdId: " << nCmdId << " timeout: " << timeout;
...
{
    qDebug() << "nCmdId: " << nCmdId << " nParam1: " << nParam1 << " timeout: " << timeout;
...
{
    qDebug() << "nCmdId: " << nCmdId << " nParam1: " << nParam1 << " nParam2: " << nParam2 << " timeout: " << timeout;
...
{
    qDebug() << "nCmdId: " << nCmdId << " nParam1: " << nParam1;
...
{
    qDebug() << "nCmdId: " << nCmdId << " nParam1: " << nParam1 << " nParam2: " << nParam2;
...
bool NDT1717MA::StartAcq()
{
    ...
    // 检查校正是否已经开启
    GetCurrentCorrectOption(m_status.SW_PreOffset, m_status.SW_Gain, m_status.SW_Defect);
    if (!m_status.SW_PreOffset || !m_status.SW_Gain || !m_status.SW_Defect)
    {
...
bool NDT1717MA::OffsetGeneration()
{
    ...
    return std::async(std::launch::async,
                      [this, nOffsetTotalFrames]()
                      {
                          int nValid{0};
                          do
                          {
                              nValid = gs_pDetInstance->GetAttrInt(Attr_OffsetValidFrames);
                              qDebug() << "nOffsetTotalFrames: " << nOffsetTotalFrames << " nValid: " << nValid;
...
std::future<bool> NDT1717MA::GainSelectAll()
{
    ...
    std::future<bool> gainSelectFuture =
        std::async(std::launch::async,
                   [this, nGainTotalFrames]()
                   {
                       int nValid{0};
                       do
                       {
...
bool NDT1717MA::DefectSelectAll(int groupIdx)
{
    ...
    int nValid{0};
    std::async(std::launch::async,
               [this, groupIdx, nExpectedValid, nDefectAllExceptedVliad, &nValid]()
               {
                   do
                   {
                       nValid = gs_pDetInstance->GetAttrInt(Attr_DefectValidFrames);
                       qDebug() << "nExpectedValid: " << nExpectedValid << " nValid: " << nValid;
    ...
    qDebug() << "groupIdx: " << groupIdx << " nValid: " << nValid << " nExpectedValid: " << nExpectedValid;
...
bool NDT1717MA::SetReceivedImage(int width, int height, const unsigned short* pData, int nDataSize)
{
    ...
    {
        qWarning() << "Invalid image parameters: width=" << width << " height=" << height << " dataSize=" << nDataSize;
        return false;
    }
    ...
    {
        qWarning() << "Image data size mismatch. Expected: " << expectedSize << " Actual: " << nDataSize;
        return false;
    }
    ...
    try
    {
        ...
        qDebug() << "Image deep copied successfully: " << width << "x" << height << " (" << nDataSize << " bytes)";
        return true;
    }
...
bool NDT1717MA::CheckBatteryStateOK()
{
    ...
    if (m_status.Battery_Exist && !m_status.Battery_ExternalPower && m_status.Battery_Remaining < MIN_REMAINING)
    {
        qDebug() << "电量低，不允许进行扫描";
        return false;
    }
...
void NDT1717MA::TryOpenCorrection()
{
    ...
    qDebug() << "校正文件检查 - 路径:" << calibPath << " Offset:" << hasOffset << " Gain:" << hasGain
             << " Defect:" << hasDefect;
