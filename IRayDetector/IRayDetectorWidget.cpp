...
IRayDetectorWidget::IRayDetectorWidget(QWidget* parent) : QMainWindow(parent), ui(new Ui::IRayDetectorWidgetClass)
{
    ui->setupUi(this);
    ...
    connect(ui->pushButton_disconnect, &QPushButton::clicked, this, [this]() { DET.DeInitialize(); });

    connect(ui->pushButton_Abort, &QPushButton::clicked, this, [this]() { DET.Abort(); });
    ...
    connect(&DET, &NDT1717MA::signalCorrectChanged, this,
            [this]()
            {
                auto status = DET.Status();
                ui->checkBox_offset->setChecked(status.SW_PreOffset);
    ...
    connect(ui->checkBox_offset, &QCheckBox::toggled, this,
            [this, updateCorrectOption](bool checked)
            {
                Q_UNUSED(checked);
                updateCorrectOption();
            });
    connect(ui->checkBox_gain, &QCheckBox::toggled, this,
            [this, updateCorrectOption](bool checked)
            {
                Q_UNUSED(checked);
                updateCorrectOption();
            });
    connect(ui->checkBox_defect, &QCheckBox::toggled, this,
            [this, updateCorrectOption](bool checked)
            {
                Q_UNUSED(checked);
                updateCorrectOption();
    ...
    connect(ui->pushButton_nVal, &QPushButton::clicked, this,
            [this]()
            {
    ...
    connect(ui->pushButton_fVal, &QPushButton::clicked, this,
            [this]()
            {
                ui->lineEdit_msg->clear();
    ...
    connect(ui->pushButton_strVal, &QPushButton::clicked, this,
            [this]()
            {
                ui->lineEdit_msg->clear();
    ...
    connect(ui->pushButton_clearAcq, &QPushButton::clicked, this, [this]() { DET.ClearAcq(); });

    connect(ui->pushButton_StartAcq, &QPushButton::clicked, this,
            [this]()
            {
                int state{-1};
                ...
                if (state == 2)
                {
                    QMessageBox::warning(nullptr, "警告", "请等待当前采集完成，或取消当前采集任务！", QMessageBox::Ok);
                    return;
                }
    ...
    connect(ui->pushButton_StopAcq, &QPushButton::clicked, this, [this]() { DET.StopAcq(); });

    connect(ui->pushButton_readImage, &QPushButton::clicked, this,
            [this]()
            {
                ...
                qDebug() << "min: " << valuesMinMax.first << " max: " << valuesMinMax.second;
    ...
    connect(ui->pushButton_saveImage, &QPushButton::clicked, this,
            [this]()
            {
                int width = 2000;
                int height = 500;
    ...
    connect(ui->pushButton_Invoke, &QPushButton::clicked, this,
            [this]()
            {
    ...
    connect(ui->pushButton_SyncInvoke, &QPushButton::clicked, this,
            [this]()
            {
                int nCmdId = ui->lineEdit_cmdId->text().toInt();
                bool hasParam1 = ui->checkBox_nParam1->isChecked();
    ...
    connect(ui->pushButton_Setnval, &QPushButton::clicked, this,
            [this]()
            {
                int nVal = ui->lineEdit_ValueToSet->text().toInt();
                int nAttrID = ui->lineEdit_nAttrIDToSet->text().toInt();
                if (!DET.SetAttr(nAttrID, nVal) || !DET.WriteUserROM())
                {
                    ui->lineEdit_msg->setText("写入错误！");
                }
            });

    connect(ui->pushButton_Setfval, &QPushButton::clicked, this,
            [this]()
            {
                float fVal = ui->lineEdit_ValueToSet->text().toFloat();
                int nAttrID = ui->lineEdit_nAttrIDToSet->text().toInt();
                if (!DET.SetAttr(nAttrID, fVal) || !DET.WriteUserROM())
                {
                    ui->lineEdit_msg->setText("写入错误！");
                }
            });

    connect(ui->pushButton_Setstrval, &QPushButton::clicked, this,
            [this]()
            {
                std::string strVal = ui->lineEdit_ValueToSet->text().toStdString();
                int nAttrID = ui->lineEdit_nAttrIDToSet->text().toInt();
                if (!DET.SetAttr(nAttrID, strVal.c_str()) || !DET.WriteUserROM())
                {
                    ui->lineEdit_msg->setText("写入错误！");
                }
            });
