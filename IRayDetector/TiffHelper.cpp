...
TiffHelper::TiffHelper(QObject* parent) : QObject(parent) {}
...
QImage TiffHelper::ReadImage(const std::string& file_path)
{
    ...
    qDebug() << "TIFF 图像信息: 尺寸=" << width << "x" << height << "位深=" << bitsPerSample
             << "通道数=" << samplesPerPixel << "压缩=" << compression;
    ...
    {
        qWarning() << "不支持的 TIFF 格式: 位深=" << bitsPerSample << "通道数=" << samplesPerPixel;
    }
...
bool TiffHelper::SaveImage(const QImage& image, const std::string& file_path)
{
    ...
    {
        qDebug() << "成功保存 TIFF 文件:" << file_path.c_str() << "尺寸:" << width << "x" << height
